...
export default function EventsPage() {
  ...
  const filteredEvents = useMemo(() => {
    // For students, only show approved upcoming events
    ...
    // Filter by institute (for students)
    let instituteFiltered = filterEventsByInstitute(baseFiltered, user);
    
    // Apply time-based filter (for admins, or if student wants to filter)
    ...
    <div>
      <header style={{ marginBottom: "1rem" }}>
        ...
        <p style={{ color: "var(--muted)" }}>
          {user?.role === "student"
            ? "Browse upcoming events you can attend."
            : "Browse and coordinate academic, extracurricular, and administrative programming."}
        </p>
      </header>
      {user?.role !== "student" && <EventFilters onFilterChange={setSelectedFilter} />}
      {loading ? (
        ...
        <div className="grid" style={{ gap: "1.25rem", marginTop: user?.role !== "student" ? "1.5rem" : "0" }}>
